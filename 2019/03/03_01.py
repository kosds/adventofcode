    ...
    for move in path:
        ...
        if move[0] == 'R':
            for y in range(pos[1] + 1, pos[1] + disp + 1):
                if check:
                    if grid[pos[0]][y] == 1:
                        inter.append((pos[0],y))
                else:
                    grid[pos[0]][y] = 1
            pos[1] += disp
        if move[0] == 'L':
            for y in range(pos[1] - disp, pos[1]):
                if check:
                    if grid[pos[0]][y] == 1:
                        inter.append((pos[0],y))
                else:
            ...
            pos[1] -= disp
        if move[0] == 'U':
            for x in range(pos[0] - disp, pos[0]):
                if check:
                    if grid[x][pos[1]] == 1:
                        inter.append((x,pos[1]))
                else:
                    grid[x][pos[1]] = 1
            pos[0] -= disp
        if move[0] == 'D':
...
def proc(line):
    ...
    for path in paths:
        ...
        checked = True
    print(inter)
    intern = [sum([abs(aa - center) for aa in a]) for a in inter]
    print(intern)
    res = min(intern)
    print(res)
